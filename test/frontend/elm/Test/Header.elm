        ...
        [ describe "update"
            [ describe "GetSearchResult"
                [ describe "parseQuery"
                    [ test "account" <|
                        \() ->
                            Expect.equal (parseQuery "123412341234") (Ok AccountQuery)
                    , test "public key" <|
                        \() ->
                            Expect.equal (parseQuery "EOS5uxjV3FYZvwqyAM2StkFEvUvf43F7gSrZcBpunuuTxiYkKqb6d") (Ok PublicKeyQuery)
                    , test "not both" <|
                        \() ->
                            Expect.equal (parseQuery "123") (Err "invalid input")
                    ]
                , describe "accountDecoder"
                    [ test "Account parsing (core_ram_liquid field doesn't exist)" <|
                        \() ->
                            ...
                            in
                                Expect.equal (JD.decodeString accountDecoder accountJson) (Ok expectedAccount)
                        ...
                        \() ->
                            let
                                ...
                                expectedAccountsList =
                                    [ "eosswedenorg" ]
                            in
                                Expect.equal (JD.decodeString keyAccountsDecoder keyAccountsJson) (Ok expectedAccountsList)
                    ]
                ]
            ]
        ]
